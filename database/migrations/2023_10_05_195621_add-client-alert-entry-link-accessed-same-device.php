...
class AddClientAlertEntryLinkAccessedSameDevice extends Migration
{
    ...
    {
        $ca = new ClientAlert();
        $ca->title = 'Link Accessed On The Same Device';
        $ca->channels = 'DTD,Retail,TM';
        $ca->description = "Customer accesses a link (signature/digital) on the same device as the sales agent.";
        $ca->threshold = 0;
        $ca->function = 'CheckLinkAccessedSameDevice';
        $ca->category_id = 5;
    ...
    {
        ClientAlert::where('function', 'CheckLinkAccessedSameDevice')->delete();
    }
}
