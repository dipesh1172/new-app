  ...
  <div id="main-app">
    <breadcrumb
      :items="[
        ...
        {name: 'IVR Report', url: '/reports/ivr_report', active: true}
      ]"
    />
      ...
      <div class="animated fadeIn">
        ...
        <div class="card mt-4">
          <div class="card-header">
            <i class="fa fa-th-large" /> Report: IVR
          </div>
          <div class="card-body">
            <custom-table
              :headers="headers"
...
export default {
    ...
    computed: {
        exportUrl() {
            return `/reports/list_ivr?${this.filterParams}${this.sortParams}&csv=true`;
        },
        sortParams() {
    ...
    mounted() {
        ...
        axios
            .get(
                `/reports/list_ivr?${pageParam}${this.filterParams}${this.sortParams}`,
            )
            .then((response) => {
    ...
    methods: {
        searchData({ startDate, endDate, brand }) {
            ...
            window.location.href = `/reports/ivr_report?${filterParams}${this.sortParams}`;
        },
        selectPage(page) {
            window.location.href = `/reports/ivr_report?page=${page}${this.filterParams}${this.sortParams}`;
        },
