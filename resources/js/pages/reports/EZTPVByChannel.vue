  ...
  <div id="main-app">
    <breadcrumb
      :items="[
        ...
        {name: 'EZTPV By Channel', url: '/reports/eztpv_by_channel', active: true}
      ]"
    />
      ...
      <div class="animated fadeIn">
        ...
        <div class="card">
          <div class="card-header">
            <i class="fa fa-th-large" /> Report: EZTPV By Channel
          </div>
          <div class="row mt-5 p-3">
            <div class="col-md-12">
              ...
              <custom-table
                ...
                empty-table-message="No eztpv were found."
...
export default {
    ...
    computed: {
        exportUrl() {
            return `/reports/list_eztpv_by_channel?${this.filterParams}${this.sortParams}&csv=true`;
        },
        sortParams() {
    ...
    mounted() {
        ...
        axios
            .get(
                `/reports/list_eztpv_by_channel?${pageParam}${this.filterParams}${this.sortParams}`,
            )
            .then((response) => {
                this.dataIsLoaded = true;
                const res = response.data;
                this.eztpvs = res.data.map((e) => {
    ...
    methods: {
        searchData({
            ...
            window.location.href = `/reports/eztpv_by_channel?${filterParams}${this.sortParams}`;
        },
        sortData(serviceKey, index) {
            this.headers[index].sorted = this.headers[index].sorted === ASC_SORTED ? DESC_SORTED : ASC_SORTED;

            this.eztpvs = arraySort(this.headers, this.eztpvs, serviceKey, index);
        ...
        selectPage(page) {
            window.location.href = `/reports/eztpv_by_channel?page=${page}${this.filterParams}`;
        },
        getParams() {
