  ...
  <div>
    <breadcrumb
      :items="[
        ...
        {name: 'Call Validation', url: '/reports/call_validation', active: true}
      ]"
    />
      ...
      <div class="animated fadeIn">
        ...
        <div class="card">
          <div class="card-header">
            <i class="fa fa-th-large" /> Call Validation
          </div>
...
export default {
    ...
    data() {
        return {
            totalRecords: 0,
            tableDataset: [],
            exportUrl: '/reports/call_validation/list_call_validation?&csv=true',
            headers: [],
            dataIsLoaded: false,
            activePage: 1,
            numberPages: 1,
    ...
    mounted() {
        ...
        this.exportUrl = `/reports/call_validation/list_call_validation?${this.filterParams}${this.sortParams}&csv=true`;
        axios
            .get(
                `/reports/call_validation/list_call_validation?${pageParam}${this.filterParams}${this.sortParams}`,
            )
            .then((response) => {
    ...
    methods: {
        searchData({ startDate, endDate }) {
            ...
            window.location.href = `/reports/call_validation?${filterParams}${this.sortParams}`;
        },
        selectPage(page) {
            window.location.href = `/reports/call_validation?page=${page}${this.filterParams}${this.sortParams}`;
        },
