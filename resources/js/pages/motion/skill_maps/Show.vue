                ...
                <div class="animated fadeIn">
                    ...
                    <div class="card mb-0">
                        ...
                        <div class="card-header skills-header">
                            ...
                            <!-- CARD HEADER TEXT -->
                            <i class="fa fa-th-large mt-3" /> Skill Maps

                            <!-- ADD SKILL MAP BUTTON -->
                            <a :href="createUrl" class="btn btn-success mr-0 mt-1 pull-right">
                                <i class="fa fa-plus" aria-hidden="true" /> Add Skill Map
                            </a>
                        ...
                        <div class="card-body p-0">
                            ...
                            <custom-table
                                :headers="headers"
                                :data-grid="this.skillMaps"
                                :data-is-loaded="dataIsLoaded"
                                show-action-buttons
                                has-action-buttons
                                empty-table-message="No skill maps were found. Create one."
                                class=""
...
const NO_SORTED = '';
const BASE_URL = '/motion/skill_maps';     // Base URL for dipslaying the page
const LIST_URL = BASE_URL + '/list';       // Base URL for fetching data

export default {
    name: 'MotionSkillsMapping',
    components: {
        Breadcrumb,
        CustomInput,
        CustomTable,
    ...
    data() {
        return {
            ...
            headers: [{
                ...
                label: 'Focus DNIS',
                key: 'focus_dnis',
                serviceKey: 'focus_dnis',
                width: '15%',
                sorted: NO_SORTED
            }, {
                label: 'Motion Skill',
                key: 'motion_skill',
                serviceKey: 'motion_skill',
                width: '15%',
                sorted: NO_SORTED
            }, {
                label: 'Motion DNIS',
                key: 'motion_dnis',
                serviceKey: 'motion_dnis',
                width: '15%'
            }],
    ...
    methods: {
        ...
        denormalizeSkillMaps({data: mapData}) {

            const skillMapResult = mapData.map((skillMap) => {
                return {
                    id:           skillMap.id,
                    brand_name:   skillMap.brand_name,
                    focus_dnis:   skillMap.focus_dnis,
                    language:     skillMap.language,
                    motion_skill: skillMap.motion_skill,
                    motion_dnis:  skillMap.motion_dnis,
                    buttons: [{
                        type: 'edit',
                        url:  `${BASE_URL}/${skillMap.id}/edit`
                    }, {
                        type:       'custom',
                        label:      'Delete',
                        icon:       'trash',
                        ...
                        url:        '#',
                        onClick: (e) => {
                            this.deleteSkillMapping(e, skillMap.id)
                        },
                    }]
                };
            });
        ...
        renderSkillMapping(skillMapData) {
            this.skillMaps = this.denormalizeSkillMaps(skillMapData);

            this.dataIsLoaded = true; // Stops the spinner in the results table
            this.activePage   = skillMapData.current_page;
            this.numberPages  = skillMapData.last_page;
        },

        // Fetch the Skill Mapping list from database
        async fetch(baseUrl, params) {
            ...
            try {
                skillMaps = await axios.get(`${LIST_URL}${params}`);

                console.log("Skill Maps", skillMaps);
                this.renderSkillMapping(skillMaps.data);
                return skillMaps;
        ...
        // Delete the selected skill mapping
        async deleteSkillMapping(event, id) {

            try {
                const c = confirm('Are you sure you want to delete this skill map?');
