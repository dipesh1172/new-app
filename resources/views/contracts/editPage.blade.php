...
@section('title')
{{ $contractConfig->contract_name }} - Page #{{ $contractConfigPage->page }}
@endsection
...
@section('content')
    <div id="contract-editor">
        <contract-editor
            ...
            :contract-config-page="{{ json_encode($contractConfigPage) }}"
        />
    </div>
@endsection
        ...
        d = {
            ...
            addCoord: function(a, b) {
                var d = c[a].state.areaIndex,
                    e = c[a].area[d].shape;
                (-1 == ["rect"].indexOf(e) || 2 != c[a].area[d].coords.length) && c[a].area[d].coords.push(b)
            },
