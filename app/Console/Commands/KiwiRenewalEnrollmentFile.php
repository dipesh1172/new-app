...
class KiwiRenewalEnrollmentFile extends Command
{
     ...
     * @var string
     */
    protected $signature = 'KiwiRenewalEnrollmentFile {--mode=} {--start-date=} {--end-date=} {--noftp} {--noemail}';
     ...
     * @var string
     */
    protected $jobName = 'Kiwi Renewal Enrollment File ';
     ...
     * @var array
     */
    protected $brandId = '31b177d0-33d6-4c51-9907-5b57f68a9526'; //  prod ID
    ...
    public function handle()
    {
        ...
        $data = StatsProduct::select(
            ...
            'stats_product.vendor_label',
            ['KIWI NY 36 Month Multi','Kiwi LiveOp Retention NY 500','Kiwi LiveOp Retention']
        )->orderBy(
            'stats_product.confirmation_code'
        )->get();
        ...
        if(count($data) === 0) {
            ...
            $message = 'There were no records to send for Kiwi Renewal Enrollment File' . $this->startDate->format('m-d-Y') . '.';
        ...
        foreach ($data as $r) {
            $ecogold_program = "";
            $custom_customer_number = "0000000";  // hydrate 0000000 into the customer number field when they do not enter one
            $customFields = [];
           if($r->custom_fields) {
                ...
                foreach($customFields AS $customField) {
                    switch(strtolower($customField->output_name)) {
                        ...
                        case 'customer_number':
                            $custom_customer_number = $customField->value;
                            break;
                        }
                }
            }
            ...
            $row = [
                'action_code' => 'NEW',
                'salesperson_code' => strtoupper($r->sales_agent_rep_id),
                'salesperson_name' => $r->sales_agent_name,    
                'voice_verif_code' => $r->confirmation_code,
                'ext_customer_id' => $r->confirmation_code,
                'date_of_sale' => $r->interaction_created_at->format('m/d/Y'),
                'cust_first_name' => $r->bill_first_name,
                'cust_middle_initial' => $r->bill_middle_name,
                'cust_last_name' => $r->bill_last_name,
                'utility_account_number' => $r->account_number1,
                'service_addr_type' => '',
                'service_addr_line_1' => $r->service_address1,
                'service_addr_apart_no' => '',
                'service_addr_line_2' => $r->service_address2,
                'service_addr_city' => $r->service_city,
                'service_addr_zipcode' => $r->service_zip,
                'service_addr_property_name' => '',
                'switch_move_in_type' => 'SWITCH',
                'out_of_cycle_switch_yn' => '',
                'out_of_cycle_switch_date' => '',
                'move_in_date' => '',
                'priority_move_in_yn' => '',
                'product_code' => $r->rate_external_id,
                'contact_first_name' => $r->auth_first_name,
                'contact_middle_initial' => '',
                'contact_last_name' => $r->auth_last_name,
                'postal_addr_line_1' => $r->billing_address1,
                'postal_addr_line_2' => $r->billing_address2,
                'postal_addr_city' => $r->billing_city,
                'postal_addr_zipcode' => $r->billing_zip,
                'contact_phone_no' => (empty(str_replace('+1','',$r->btn)) ? ' ' : str_replace('+1','',$r->btn)),
                'contact_alt_phone_no' => '',
                'contact_mobile_no' => '',
                'contact_email_address' => strtoupper($r->email_address),
                'contact_password' => '',
                'date_of_birth' => '',
                'preferred_contact_method' => 'PHONE',
                'preferred_language' => ($r->language == 'Spanish' ? 'S' : 'E'),
                'SSN' => '',
                'drivers_license' => '',
                'alt_identification' => '',
                'state' => $r->service_state,
                'service_type' => ($r->commodity == 'Electric' ? 'E' : 'G'),
                'LOA' => '',
                'utility_code' => $r->utility_commodity_external_id,
                'ecgold_program_code' => $ecogold_program,
                'promo_code' => ($r->product_name == 'Kiwi Homeguard' ? '' : ''),
                'rate_promo' => '',
        ...
        if (!$this->option('noemail')) {
            ...
            $this->sendEmail('Attached is the file for Kiwi Enrollment File' . $this->startDate->format('m-d-Y') . '.', $this->distroList['emailed_file'][$this->mode], $attachments);
        }
