...
class FrontierTXRateImport extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'FrontierTX:rate:import {--file=} {--groupname}';
    // protected $signature = 'rate:import {--brand=} {--file=} {--groupname}';
     ...
     * @var string
     */
    protected $description = 'Custom Rate Import process for FrontierTX, expected to be v2 generic rate:import';
    ...
    public function handle()
    {
        ...
        $brand = Brand::where('name', 'Frontier Utilities LLC TX')->whereNotNull('client_id')->first();
        if (!$brand) {
            $this->error("Unable to find Brand entry for Frontier Utilities LLC TX");
            return -1;
        }
        ...
        // specific headers of extra fields for FrontierTX
        $headers_extra = [
            'Minimum Fee',
            'Utility Monthly Fee',
            'Example Threshold',
            'Example Threshold Rate Amount',
            'Tier Two Threshold',
            'Tier Two Rate Amount',
            'Bill Credit',
            'Move-In Switch Fee'
        ];
