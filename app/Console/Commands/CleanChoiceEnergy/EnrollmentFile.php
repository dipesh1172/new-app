 ...
 * 
 */
class EnrollmentFile extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cleanchoiceenergy:reports:enrollmentfile {--no-email} {--show-sql} {--start-date=} {--end-date=}';
     ...
     * @var string
     */
    protected $description = 'Enrollment file for Cleanchoice Energy';
     ...
     * @var array
     */
    protected $distroList = ['megavolt+TrustedTPV@ethicalelectric.com', 'alex.kolosha@answernet.com', 'accountmanagers@answernet.com'];
    ...
    public function handle()
    {
        ...
        $fileName = 'CleanChoice Energy Enrollment File ' . $this->endDate->format('Ymd') . '.csv';

        // Data layout/File header for TXT enrollment file
        $csvHeader = [
            'CUSTOMER_FIRST_NAME', 'CUSTOMER_LAST_NAME', 'CONTACT_FIRST_NAME', 'CONTACT_LAST_NAME', 'UTILITY', 'ACCOUNT_NUMBER',
            'PLAN_NAME', 'PLAN_CODE', 'RATE_TYPE', 'RATE', 'STATE', 'SUB_ACCOUNT_NO', 'CDR_ANI', 'TRANSACTION_ID', 'DATE_TIME',
            'DURATION', 'REP_ID', 'BTN', 'VERIFIED', 'CANCELED_REASON', 'CANCELED_REASON_TEXT', 'COMMENTS'
        ];
        ...
        $data = StatsProduct::select(
            ...
            DB::raw('IF(stats_product.result = "Sale",  "", stats_product.disposition_label) AS CANCELED_REASON'),
            DB::raw('IF(stats_product.result = "Sale",  "", stats_product.disposition_reason) AS CANCELED_REASON_TEXT'),
        ...
        foreach ($data as $r) {
            ...
            // Exclude No Sale - Pending (EZTPV submissions have were not called in on yet)
            if($r->interaction_type == 'eztpv' && $r->result == 'No Sale' && $r->disposition_reason == 'Pending') {
                continue;
            }

            $verified = '';
            if($r->interaction_type == 'ivr_script' && $r->result == 'No Sale' && $r->disposition_reason == 'Incomplete IVR') {
                $verified = 'I';
            } else if($r->result == 'Sale') {
                $verified = 'Y';
            } else {
                $verified = 'N';
            }
            ...
            $row = [
                ...
                'CANCELED_REASON_TEXT' => $r->CANCELED_REASON_TEXT, 
                'COMMENTS' => $comments,
            ];
    ...
    public function sendEmail(string $message, array $distro, array $files = array())
    {
        ...
        if ('production' != config('app.env')) {
            $subject = 'CleanChoice Energy - Enrollment File Generation (' . config('app.env') . ') '
                . Carbon::now('America/Chicago');
        } else {
            $subject = 'CleanChoice Energy - Enrollment File Generation '
                . Carbon::now('America/Chicago');
        }
