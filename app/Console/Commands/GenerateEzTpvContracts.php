...
class GenerateEzTpvContracts extends Command
{
     ...
     * @var string
     */
    protected $signature = 'eztpv:generateContracts
        {--eztpv_id=} 
        {--confirmation_code=} 
        {--hoursAgo=}
        {--debug}
    ...
    */
        ...
        if ($eps) {
            ...
            foreach ($eps as $ep) {
                ...
                $rateInfo = Product::select(
                    ...
                    ->where('rates.id', $ep['rate_id'])
                    ->withTrashed()->first();

                $product['name'] = $rateInfo->name;
                $product['rate_type_id'] = $rateInfo->rate_type_id;
                $product['rate_amount'] = $rateInfo->rate_amount;
                $product['intro_rate_amount'] = $rateInfo->intro_rate_amount;
                $product['event_type_id'] = $ep['event_type_id'];
                $product['utility_id'] = $rateInfo->utility_id;
                $product['promotion'] = [
                    'name_english' => empty($ep['promotion']['name']) ? '' : $ep['promotion']['name'],
                    'name_spanish' => empty($ep['promotion']['name_spanish']) ? '' : $ep['promotion']['name_spanish'],
                    'type' => empty($ep['promotion']['promotion_type']) ? '' : $ep['promotion']['promotion_type'] . '',
                    'code' => empty($ep['promotion']['promotion_code']) ? '' : $ep['promotion']['promotion_code'],
                    'key' => empty($ep['promotion']['promotion_key']) ? '' : $ep['promotion']['promotion_key'],
                    'reward' => empty($ep['promotion']['reward']) ? '' : $ep['promotion']['reward'] . '',
                    'text_english' => empty($ep['promotion']['promo_text_english']) ? '' : $ep['promotion']['promo_text_english'],
                    'text_spanish' => empty($ep['promotion']['promo_text_spanish']) ? '' : $ep['promotion']['promo_text_spanish'],
                ];
                $product['promotion_id'] = $ep['brand_promotion_id'];
    ...
    public function generate_contract_document($eztpv, $data, $contract_id = null)
    {
        ...
        for ($x = 0; $x <= count($data['finalized_products']) - 1; ++$x) {
            ...
            unset(
                ...
                $date,
                $date_plus_3_days,
                $date_plus_7_calendar_days,
                $date_plus_10_days,
                $date_plus_30_calendar_days,
                $delivery_method,
                $eft_radio,
            ...
            $date_plus_7_calendar_days = date('m-d-Y', strtotime($eztpv->created_at . '+ 7 days'));
            $date_plus_10_days = date('m-d-Y', strtotime($eztpv->created_at . '+ 10 weekdays'));            
            $date_plus_30_calendar_days = date('m-d-Y', strtotime($eztpv->created_at . '+ 30 days'));
            ...
            $phone_number = (!empty($data['phone_number']))
                ...
                : null;
            $rate_info = Rate::select('products.term', 'rates.rate_amount', 'rates.rate_source_code')
                ->join('products', 'rates.product_id', 'products.id')
                ->where('rates.id', $data['finalized_products'][$x][0]['rate_id'])
                ->withTrashed()
                ->first();
            if (!$rate_info) {
                $this->info('Rate ' . $data['finalized_products'][$x][0]['rate_id'] . ' is deleted or otherwise invalid');
                $this->logError('Rate is deleted or invalid.  Skipping...', __METHOD__, __LINE__);
                continue;
            }

            $rate_info_term = $rate_info->term;
            $rate_info_rate_amount = $rate_info->rate_amount;
            $rate_info_rate_source_code = $rate_info->rate_source_code;
            $rate_info_electric = '';
            $rate_info_gas = '';
            ...
            foreach ($data['finalized_products'][$x] as $product) {
                ...
                if ($event_type && 'Electric' == $event_type->event_type) {
                    ...
                    if (isset($utility_electric)) {
                        $utility_electric_primary_identifier = $this->getUtilityPrimaryIdentifier($this->eztpv->event, $utility_electric);
                ...
                if ($event_type && 'Natural Gas' == $event_type->event_type) {
                    ...
                    if (isset($utility_gas)) {
                        $utility_gas_primary_identifier = $this->getUtilityPrimaryIdentifier($this->eztpv->event, $utility_gas);
            ...
            if ($contract_id) {
                ...
                switch ($this->eztpv->brand_id) {
                    ...
                    case '293c51ca-87de-41c6-bb98-948c7537bc11':
                        ...
                        $atlanticProducts = [
                            'ea72c85f-2893-4fdd-bd77-fad26a625679',  // 'OH 12 Month Smart Rate Intro to Variable' - PROD Electric Only Contract
                            '46b548b8-ef81-40d8-8e63-b4d379c658e6'   // 'OH 12 Month Smart Rate Intro to Variable' - Stage Electric Only Contract
                        ];

                        // Check for Product Specific Contracts (similar to Rushmore)
                        if (in_array($commodity, ['dual','electric']) && isset($product['rate_id'])) {
                            $atlantic_rate = Rate::where('id', $product['rate_id'])->withTrashed()->first();

                            if (
                                isset($atlantic_rate)
                                && isset($atlantic_rate->product_id)
                                && in_array($atlantic_rate->product_id, $atlanticProducts)
                            ) {
                                // select by products.id
                                $pdf_info = $pdf_info->where(
                                    'product_id',
                                    $atlantic_rate->product_id
                                );
                            } else {
                                // do not select by product_id
                                $pdf_info = $pdf_info->whereNull(
                        ...
                        else {  // Commodity is Gas, do not use Product Specific Contracts
                            // do not select by product_id
                            $pdf_info = $pdf_info->whereNull(
                                'product_id'
                            );
    ...
    private function getUtilityPrimaryIdentifier($event, $utility): string
    {
        $translation = [
            'Account Number' => 'Número de cuenta',
            'Service Agreement ID' => 'ID del contrato de servicio',
            'Customer Number' => 'Número de cliente',
            'Pod ID' => 'Pod ID',
            'Service Delivery Identifier' => 'Identificador de prestación de servicios',
            'Service Number' => 'Número de servicio',
            'Meter Number' => 'Número de medidor',
            'Service Reference Number' => 'Número de referencia del servicio',
            'Name Key' => 'Clave de nombre',
            'Billing Account Number' => 'Número de cuenta de facturación',
            'Service Point ID' => 'ID de punto de servicio',
            'ESI ID' => 'ESI ID',
            'Site ID' => 'ID del sitio',
            'Choice ID' => 'ID de elección',
            'Supplier Agreement ID' => 'ID del acuerdo con el proveedor',
            'Service Agreement Number' => 'ID del acuerdo con el proveedor',
            'Device ID' => 'ID del dispositivo'
        ];

        $account_type = $utility->account_type;
        $language_id = $event->language_id;

        if($language_id == 2 && array_key_exists($account_type, $translation)) {
            return $translation[$account_type];
        }
        return $account_type;
    }
}
