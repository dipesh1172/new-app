...
class RpaWelcomeVariable extends Command
{
    private $brand_id = null;
    private $defaultVendor = 'TM Fulfillment';
    private $defaultOffice = 'TM Fulfillment';
    private $noticePrepend = 'Green Choice Energy Variable Letter';
    private $env = 'staging';
    private $errors = [];
     ...
     * @var string
     */
    protected $signature = 'RpaWelcomeVariable {--debug} {--file=} {--single} {--dryrun}';
     ...
     * @var string
     */
    protected $description = 'RPA Variable Letter job';
                    ...
                    try {
                        ...
                        if ( // Don't process records with missing address
                            ...
                            if (!$new_event) {
                                ...
                                $this->addCustomField($this->brand_id, $event->id, "month", $data['month']);
                                        ...
                                        if (isset($data['email_address'])) {
                                            try {
                                                ...
                                                if (!$this->option('dryrun')) {
                                                    $subject = "RPA Welcome Variable Letter";
                                    ...
                                    case 'text':
                                        ...
                                        if (isset($data['btn'])) {
                                            if ($event->language_id == 2) {
                                                $message = config('app.urls.clients') . '/rpa/welcomevariable/' . $event->id . '/' . $email_address . '/' . $account_number
    ...
    public function ftpDownload($config)
    {
        ...
        try {
            ...
            try {
                $this->info("Checking if file " . $filename . " exists for letter");
                $exists = $filesystem->has($filename);
                if (!$exists) {
                    $msg = 'Checking if client placed Welcome Variable Letter Filename: ' . $filename . ' for processing. File not exist on FTP No issue';
                    ...
                    if ($contents) {
                        ...
                        $responseCode = $filesystem->rename($filename,'processed_'.$filename);    
