...
namespace App\Console\Commands\Marathon;
...
class DailyCallsReport extends Command
{
     ...
     * @var string
     */
    protected $signature = 'marathon:reports:daily-calls:email {--mode=} {--env=} {--no-email} {--show-sql} {--start-date=} {--end-date=}';
     ...
     * @var string
     */
    protected $description = 'Daily calls for Marathon Power LLC';
     ...
     * @var string
     */
    protected $brand = 'Marathon Power LLC';
     ...
     */
    protected $brandId = [
        'prod' => '752d40bf-8775-478c-ab7b-3f1b5ea41efb',
        'stage' => 'x' // At the time this report was written, the brand only existed in prod. Setting to 'x' to ensure not data gets pulled for staging runs.
    ];
    ...
    protected $distroList = [
        'live' => ['cskulikidis@mecny.com', 'nick@mecny.com', 'mruan@mecny.com', 'dxc_autoemails@tpv.com'], // TODO: Replace with live distro list
        'test' => ['alex.kolosha@answernet.com']
    ];
...
/**
    ...
    public function handle()
    {
        ...
        foreach ($data as $r) {
            ...
            foreach($customFields AS $customField) {
                ...
                if($customField->output_name == 'rate_amount' && $customField->product_id && $customField->product_id == $r->event_product_id) {
                    $rateAmount = $customField->value;                    
                }

                if($customField->output_name == 'term_length' && $customField->product_id && $customField->product_id == $r->event_product_id) {
                    $termLength = $customField->value;                    
                }
            }

            $row = [
                'acct_num'            => $r->acct_num,
                'confirmation_num'    => $r->confirmation_num,
                'tpv_num'             => substr($r->tpv_num, 2), // remove +1
                'ani'                 => $r->ani,
                'rep_id'              => $r->rep_id,
                'Prod'                => $r->prod,
                'cust_phone_num'      => $r->cust_phone_num,
                'dt'                  => $r->date_time,
                'dur'                 => round($r->duration * 60),
                'complete'            => $complete,
                'svc_street1'         => $r->svc_street1,
                'svc_street2'         => $r->svc_street2,
                'svc_city'            => $r->svc_city,
                'svc_state'           => $r->svc_state,
                'svc_zip'             => $r->svc_zip,
                'utility'             => $r->utility,
                'rate'                => $rateAmount,
                'term'                => $termLength,
                'reject_reasons'      => ($r->interaction_type == 'ivr_review'? $r->disposition_reason : ''),
    ...
    private function getData() 
    {
        $data = StatsProduct::select(
            'stats_product.account_number1 AS acct_num',
            'stats_product.confirmation_code AS confirmation_num',
            'stats_product.dnis AS tpv_num',
            'stats_product.ani AS ani',
            'stats_product.sales_agent_rep_id AS rep_id',
            'stats_product.product_name AS prod',
            'stats_product.btn AS cust_phone_num',
            'stats_product.interaction_created_at AS date_time',
            'stats_product.interaction_time AS duration',
            'stats_product.service_address1 AS svc_street1',
            DB::raw("'' AS svc_street2"),
            'stats_product.service_city AS svc_city',
            'stats_product.service_state AS svc_state',
            'stats_product.service_zip AS svc_zip',
            'stats_product.utility_commodity_ldc_code AS utility',
            'stats_product.product_rate_amount AS rate',
            'stats_product.product_term AS term',
            'stats_product.interaction_type',       // Needed for field mapping logic
            'stats_product.result',                 // Needed for field mapping logic
            'stats_product.disposition_reason',
            'interactions.notes AS interaction_notes',
            'stats_product.event_product_id'        // Needed to parse product level custom fields
        )
        ->leftJoin('rates', 'stats_product.rate_id', 'rates.id')
        ->leftJoin('interactions', 'stats_product.interaction_id', 'interactions.id')
        ->leftJoin('vendors', 'stats_product.vendor_id', 'vendors.vendor_id') // In stats_product, vendor_grp_id is an in. Pull directly from vendors table in cause of alpha-numeric values
        ->whereDate(
