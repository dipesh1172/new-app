 ...
 */
class GenieStarfieldOHEnrollmentFile extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'Genie:Starfield:OH:EnrollmentFile 
     ...
     * @var string
     */
    protected $description = 'Starfield OH emailed enrollment file';
    ...
    public function handle()
    {
        ...
        $fileName = 'TPV.COM_DailySales_4_' . $this->endDate->format('Ymd') . '.csv';
        ...
        $data = $data->where(
            ...
            "service_state IN ('OH') AND product_utility_external_id IN ('VEDO', 'DEOHG', 'DPL') AND ISNULL(contracts)" 
        )->orderBy(
            'interaction_created_at'
        );
        ...
        foreach ($data as $r) {
            ...
            if(!$this->option('resubmit')) {
                $jd = JsonDocument::where('document_type', 'genie-starfield-oh-enrollment')
            ...
            if(!$this->option('no-json-doc')) {
                ...
                $jd->document_type = 'genie-starfield-oh-enrollment';
    ...
    public function sendEmail(string $message, array $distro, array $files = array())
    {
        ...
        if ('production' != config('app.env')) {
            $subject = $subjectBrand . ' - OH - D2D Email File Generation - Starfield (' . config('app.env') . ') '
                . Carbon::now('America/Chicago');
        } else {
            $subject = $subjectBrand . ' - OH - D2D Email File Generation - Starfield '
                . Carbon::now('America/Chicago');
        }
