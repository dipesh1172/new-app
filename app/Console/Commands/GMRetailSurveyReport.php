...
class GMRetailSurveyReport extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'gm:retailsurvey:report {--brand=} {--script=} {--emailto=} {--debug}';
     ...
     * @var string
     */
    protected $description = 'GM Retail Survey Report';
...
//            print_r($results->toArray());
 //          exit();
            ...
            // ALLTRIM(STR(rec_id)) + 'GMRetail' AS rec_id ;
                   ...
                   foreach ($data_array as $result) {
                        if ($state_sav !== $result['state_abbrev']) {
                            if ($state_sav == 'XX') {
                                ...
                                $filename = 'gm_res_dtd_retail_' . $state_sav . '_accumulation_report_' . date("Y_m_d_",time()) . time() . '.csv';
                                ...
                                if ($state_sav !== 'XX') {
                                    fclose($file);
                                    $subject = 'GM - Res - DTD - Retail - Accumulation - Nightly State Report for ' .  $state_sav . ' ' . date('m-d-Y H:i:s');
