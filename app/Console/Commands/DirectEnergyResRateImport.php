...
class DirectEnergyResRateImport extends Command
{
     /**
     ...
     * @var string
     */
    protected $signature = 'DirectEnergy:Res:Rate:Import {--mode=} {--date=} {--file-num=} {--dry-run} {--local-file}';
     ...
     * @var string
     */
    protected $description = 'Imports a Direct Energy rate files, for the residential channel.';
    ...
    public function handle()
    {
        ...
        if(count($errors) > 0) {
            ...
            $file = fopen($this->workDir . 'de_res_rate_import_errors.csv', "w");
     ...
     */
    protected function importRecord(object $line) {
     ...
     */
    protected function findOrCreateProduct(object $line) {
    ...
    protected function buildProductName($line) {
        ...
        //   - Customer Type     -- 'Res' for residential, this case
        ...
        $term = $this->extractTermLength($line->rate_script_eng);
        $productName = 'Discovery - Res - ' . $line->sales_state . ' - ' . $line->esco_rate_desc . (!empty($term) ? ' - ' . $term : '');
     ...
     * @param object $line - The line from the rate file.
     */
    protected function createRate(string $productId, string $utilityId, object $line) {
     ...
     */
    protected function mapExtraFields(array $fields, object $line) {
    ...
    protected function setup() {
        ...
        // Set filenames
        // Default filename: gateway_res_rates_YYYYMMDD_001.zip/txt
        ...
        $this->zipFilename = "gateway_res_rates_" . $fileDate . "_" . $fileNum . ".zip"; // TODO: Remove _test
        $this->txtFilename = "gateway_res_rates_" . $fileDate . "_" . $fileNum . ".txt";
    ...
    protected function deleteOldRates() {
        ...
        // Find the old rates.
        // Look for rates tagged for residential customers only
