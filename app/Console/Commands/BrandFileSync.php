...
//                    $filename = $dd['btn'] . '_' . $confirmation_code . '_' . $type . '.mp3';
        ...
        if (!$useCurl && !$useS3) {
            ...
            try {
                ...
                if (!$this->option('noSlack')) {
                    SendTeamMessage('focus-monitoring-csm', '[BrandFileSync][' . $brand->name . '] - unable to connect to ' . $config['host'] . ' Error: ' . $e->getMessage());
                }
            ...
            if($tpvhubFtpConfig) {
                ...
                try {
                    ...
                    if (!$this->option('noSlack')) {
                        SendTeamMessage('focus-monitoring-csm', '[BrandFileSync][' . $brand->name . '] - unable to connect to ' . $tpvhubFtpConfig['host'] . ' Error: ' . $e->getMessage());
                    }
        ...
        foreach ($summary as $key => $value) {
            ...
            foreach ($value as $confirmation_code => $array) {
                ...
                try {
                            ...
                            if ($stream = fopen('/tmp/files/' . $local_filename, 'r+')) {
                                ...
                                if ($useCurl) {
                                    ...
                                    try {
                                        ...
                                        SendTeamMessage('monitoring', '[BrandFileSync][' . $brand->name . '] Error uploading file to ' . $remote_filename . ': ' . $e->getMessage());
                    ...
                    if ($markAsSynced) {
                        $event = Event::find($array[0]['event_id']);
    ...
    private function awsAssumeIamRole($brand, $config)
    {
        try {
            ...
            SendTeamMessage('monitoring', '[BrandFileSync][' . $brand->name . '] Error Assuming AWS IAM Role.');
        }
    ...
    private function awsCreateS3Client($brand, $config, $iamRole = null)
    {
        try {
            ...
            SendTeamMessage('monitoring', '[BrandFileSync][' . $brand->name . '] Error Creating S3 Client.');
        }
                ...
                try {
                    ...
                    if (!$this->option('state')) {
                        if ($this->option('startDate') && $this->option('endDate')) {
                            ...
                            $eventsTotal = Event::where(
                                ...
                                'created_at',
                                '>',
                                Carbon::now('America/Chicago')->startOfDay()->subMonths(3)
                            )->count();
                    ...
                    if (strlen(trim($brand->recording_transfer_config)) > 0) {
                        ...
                        if ($this->option('startDate') && $this->option('endDate')) {
                            ...
                            if ($brand->recording_transfer_type === 1) { // All files up to 30 days ago
                                $events = $events->where(
                                    'events.created_at',
                                    '<',
                                    Carbon::now()->subMonth()
                            ...
                            // Look back no more than 90 days
                            $events = $events->where(
                                'events.created_at',
                                '>',
                                Carbon::now('America/Chicago')->startOfDay()->subMonths(3)
